...
public class Searcher {
    ...
    private PostingsList rankedRetrieval(List<Query.QueryTerm> queryTerms) {
        ...
        for (int i = 0; i < queryTerms.size(); i++) {
            terms.add(queryTerms.get(i).term);
        }
        ...
        for (int i = 0; i < queryTerms.size(); i++) {
            ...
            for (int j = 0; j < allDocuments.size(); j++) {
                PostingsEntry document = allDocuments.get(j);
                double score = computeTfIdf(document, term);
        ...
        for (Map.Entry<Integer, Double> entry : docScores.entrySet()) {
            ...
            double normalizedScore = rawScore / index.docLengths.get(docID);

            PostingsEntry newEntry = new PostingsEntry(docID);
            newEntry.setScore(normalizedScore);
            results.insertEntry(newEntry);
        }
    ...
    private double computeTfIdf(PostingsEntry document, String term) {
        int tf = document.positions.size();
        double df = index.getPostings(term).size();
        double N = index.docNames.size();
        double idf = Math.log(N / df);
        System.out.printf("idf - %s: %.4f\n", term, idf);
        return tf * idf;
    }
